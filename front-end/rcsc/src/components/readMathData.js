        ...
        for (let i = 0; i < answered_question_id.length; i++) {
            ...
            for (let j = 0; j < 4; j++) {
                if (j === parseInt(answered_question_info[i][2])) {
                    $(id).find("form").find(answer_class).css({
                        color: "black",
                        background: "rgba(50, 232, 50, 0.46)",
                ...
                <Row style={{padding: "10px"}}>
                                ...
                                <li id={data.id}>
                                    <div>
                                        <Row>
                                            ...
                                            <Col md={1} lg={1} sm={12}>
                                                {data.Marked ? (
                                                    <button type='button' className={"markedButton"} id={"mark" + index}
                                                            onClick={this.handleUnmark(index, data.id)}>
                                                        Unmark
                                                    </button>
                                                ) : (
                                                    <button type='button' className={"markButton"} id={"mark" + index}
                                                            onClick={this.handleMark(index, data.id)}>
                                                        mark
                                                    </button>
                                                )}
                                            </Col>
                    ...
                    <Form className={data.id}>
                      {data.Choice.map((choice, index) => (
                          <p className={index}>
                              <input type='radio' id={data.CorrectAnswer} name='choice' value={data.id}
                                     onChange={this.handleChange(data.id, choice, data.CorrectAnswer, index)}/>
                              &nbsp;&nbsp;&nbsp;&nbsp; {choice}
                          </p>
                      ))}
                    </Form>
                      <br/>
                      <span style={{display: this.state.showResult ? "block": "none"}}>
                        <h6 className={"trial_image_class"} style={{display: "none", color: "blue"}}>
                            Answer / Explanation
                        </h6><hr/>
                        <img src={data.explanationURL} alt='answer' className={"trial_image_class"}
                             style={{width: "100%"}}/>
                        <hr/>
                      </span>
                  </span>
